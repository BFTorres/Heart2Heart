...
export type CookieConsent = {
  ...
  analytics: boolean;
  /**
   * We treat this as "External media" (maps/embeds) to avoid adding a new schema field.
   * (Label + help text in i18n makes this explicit.)
   */
  marketing: boolean;
};
...
/**
 * Bump version whenever semantics change (e.g. "marketing" -> "external media").
 * This forces re-consent, which is the safer path.
 */
...
function clearConsent() {
  ...
  window.dispatchEvent(
    new CustomEvent(COOKIE_CONSENT_CHANGED_EVENT, { detail: null }),
  );
}
...
/**
 * Used by the map gate: user explicitly wants to load external content (map tiles),
 * so we store "marketing" = true (labeled as "External media" in UI).
 */
export function grantExternalMediaConsent() {
        ...
        <div className="mt-6 grid gap-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {t("cookies.settings.necessary.help")}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {t("cookies.settings.analytics.help")}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {t("cookies.settings.marketing.help")}
              </p>
            </div>
          ...
          {consent?.decidedAt ? (
            <p className="pt-2 text-xs text-muted-foreground">
              {t("cookies.settings.lastUpdated", {
                date: new Date(consent.decidedAt).toLocaleString(),
              })}
            </p>
          ) : null}
        </div>
