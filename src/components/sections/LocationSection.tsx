...
export function LocationSection() {
  ...
  const mapTheme =
    theme === "dark" || theme === "contrast-dark" ? "dark" : "light";

  // External media consent (we use consent.marketing as "External media")
  const [externalAllowed, setExternalAllowed] = useState<boolean>(() => {
    const c = readConsent();
    return !!c?.marketing;
  });
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      setExternalAllowed(!!ce.detail?.marketing);
    };
  ...
  const destination = useMemo(() => {
    ...
    // Keep destination clean; venueName can help matching. Avoid "1. OG" etc. in destination string.
    return venueName
      ? `${venueName}, ${addressLine1}, ${postalCity}, ${country}`
      : `${addressLine1}, ${postalCity}, ${country}`;
  }, []);

  const { lat, lng, name, addressLine1, postalCity, venueName, floor } =
    SITE.location;

  const venueLabel = String(t("sections.location.labels.venueLabel"));
  const floorLabel = String(t("sections.location.labels.floorLabel"));
              ...
              {!externalAllowed ? (
                <div className="flex h-full w-full flex-col items-center justify-center gap-3 bg-muted/30 p-6 text-center">
                  <p className="text-sm font-medium">
                    {t("sections.location.mapBlockedTitle")}
                  </p>
                  <p className="max-w-md text-sm text-muted-foreground">
                    {t("sections.location.mapBlockedBody")}
                  </p>
                  ...
                  <div className="mt-2 flex flex-col gap-2 sm:flex-row">
                    <Button
                      onClick={() => {
                        grantExternalMediaConsent();
                        // state updates via event; user can still review in settings
                      }}
                      type="button"
                    >
                      {t("sections.location.loadMapCta")}
                    </Button>
                    ...
                    <Button
                      ...
                      type="button"
                    >
                      {t("sections.location.openCookieSettingsCta")}
                    </Button>
                  </div>
                </div>
              ) : (
